...
for s in df['sentiment'].unique():
    text = " ".join(df[df['sentiment'] == s]['selected_text'].astype(str).tolist())# Combine all text for the sentiment
    ...
    plt.title(f'Word Cloud for {s} Sentiment') # Title for the plot
